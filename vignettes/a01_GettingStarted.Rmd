...
```{r}
mpm1  # a stage-structured MPM for a plant
...
age-specific mortality/survival, whether proportional survivorship (lx), survival 
probability (px) or mortality hazard (hx), following an `x_to_y` function name 
pattern.

```{r}
lx_to_px(lx)  # survivorship to survival probability
lx_to_hx(lx)  # survivorship to mortality hazard
```
...
towards the SSD. Using `qsd_converge`, we can find the time at which the cohort 
reaches the QSD and subset the life table or any component age-specific trajectory
accordingly.

```{r, warning=FALSE, message=FALSE, fig.align='center', fig.height=5, fig.width=6}
# project a germinated cohort through the U matrix
cohort <- popbio::pop.projection(mpm1$matU, n = c(0, 1, 0, 0, 0), iterations = 10)
popbio::stage.vector.plot(cohort$stage.vectors[-1,], xlab = "time",
                          ylab = "proportion in stage class", col = "black")

# calculate time to QSD from the U matrix of an MPM
(q <- qsd_converge(mpm1$matU, start = "small"))

# subset the life table to ages prior to the QSD
lt_preQSD <- lt[1:q,]
...
typically applied at the level of individual matrix elements and which may be by 
a fixed or proportionate amount (**sensitivities** and **elasticities**, 
respectively). By extension, the vital rates (survivial, growth, etc.) or 
transition type (stasis, retrogression, etc.) underlying multiple matrix elements 
may be similarly perturbed. `Rage` provides a functions for each scope of 
...
MPM-based analyses take the required submatix (or submatrices), optionally allowing 
...
entropy_k(lx)       # calculate Keyfitz' entropy
```
...
stage names using the function `nameStages`, electing to use generic or custom 
names.

```{r}
(col1_auto <- nameStages(col1, prefix = "class_"))  # automated stage naming
(col1_cust <- nameStages(col1, names = c("seed", "active", "dormant"), prefix = NULL))  # overwrite with custom stages
```
