...
globalDist_longevity <- ggplot(mammals, aes(Lon, Lat)) +
  ...
  geom_point(aes(colour = log10(longevity)), alpha = 0.4, size = 0.75) +
  scale_colour_viridis_c(option = "C", name = expression(log[10] * "(longevity, yrs)")) +
...
longevity_lat <- ggplot(mammals, aes(x = abs(Lat), y = log10(longevity))) +
  ...
  labs(x = "Absolute latitude", y = expression(log[10] * "(longevity, yrs)")) +
  theme_minimal() +
  geom_smooth()
...
longevity_ecoregion <- ggplot(mammals, aes(
  ...
  labs(x = "Ecoregion", y = expression(log[10] * "(longevity, yrs)")) +
  coord_flip() +
  theme_minimal()
...
library(patchwork)
globalDist_longevity
ggsave("fig02_globalDistribution.png", width = 16, height = 8, units = "cm")
```
