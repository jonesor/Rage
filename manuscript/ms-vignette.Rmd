...
Given that we are only interested in mammals, we can explore the taxonomic groups available in the dataset.

```{r taxonomic table}
table(comadre$Class)
```

We can see here that C0MADRE contains information for many different taxonomic groups. However, in this case we are only interested in mammals. The functionality in `Rcompadre` and its data class allows us to subset the database easily. We can simply use the function `filter` from `dplyr` as if our `cdb` object was a data frame.
