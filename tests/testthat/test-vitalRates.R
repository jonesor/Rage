...
vrOut_ss <- vitalRates(matU = mat_u,matF = mat_f,splitStages = "all")

expect_type(vrOut_ss, "list")
expect_length(vrOut_ss, 5)
expect_equal(vrOut_ss$clo, 0)

vrOut_ss2 <- vitalRates(matU = mat_u,matF = mat_f,splitStages = "ontogeny")

expect_type(vrOut_ss2, "list")
expect_length(vrOut_ss2, 9)
expect_equal(vrOut_ss2$cloAdu, 0)

vrOut_ss3 <- vitalRates(matU = mat_u,matF = mat_f,splitStages = "matrixStages",matrixStages = c("active","active","active","dorm"))

expect_type(vrOut_ss3, "list")
expect_length(vrOut_ss3, 10)
expect_equal(vrOut_ss3$clo, 0)

vrOut_wt <- vitalRates(matU = mat_u,matF = mat_f,weights = "SSD")
