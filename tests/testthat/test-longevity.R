...
test_that("longevity works correctly", {
  ...
  x_noconverg <- suppressWarnings(longevity(mat_u_singular, startLife = 3))
...
test_that("longevity warns and fails gracefully", {
  ...
  expect_warning(longevity(mat_u_singular, startLife = 3))
  expect_error(longevity(mat_u_na))
  expect_error(longevity(mat_u, startLife = 10))
  expect_error(longevity(mat_u, lxCrit = 10))
})
